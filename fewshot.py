...
parser.add_argument("--model_name_or_path", default='/home/mjy/wangye/pic_prompt/PCN_clickbait/model/bert_base_chinese')
...
if  args.dataset == "zongxiang":
    dataset['train'] = CnClickbaitProcessor().get_train_examples("datasets/TextClassification/zongxiang/")
    dataset['test'] = CnClickbaitProcessor().get_test_examples("datasets/TextClassification/zongxiang/")
    class_labels = CnClickbaitProcessor().get_labels()
    scriptsbase = "TextClassification/zongxiang"
    scriptformat = "txt"
    cutoff=0.5
    max_seq_l = 250
    batch_s = args.batch_size
elif  args.dataset == "tencent":
    dataset['train'] = CnClickbaitProcessor().get_train_examples("datasets/TextClassification/tencent/")
    dataset['test'] = CnClickbaitProcessor().get_test_examples("datasets/TextClassification/tencent/")
    class_labels = CnClickbaitProcessor().get_labels()
    scriptsbase = "TextClassification/tencent"
    scriptformat = "txt"
    cutoff=0.5
    max_seq_l = 250
    batch_s = args.batch_size
elif  args.dataset == "wangyi":
    dataset['train'] = CnClickbaitProcessor().get_train_examples("datasets/TextClassification/wangyi/")
    dataset['test'] = CnClickbaitProcessor().get_test_examples("datasets/TextClassification/wangyi/")
    class_labels = CnClickbaitProcessor().get_labels()
    scriptsbase = "TextClassification/wangyi"
    scriptformat = "txt"
    cutoff=0.5
    max_seq_l = 250
    batch_s = args.batch_size
...
if args.verbalizer == "kpt":
    ...
    myverbalizer = KnowledgeableVerbalizer(tokenizer, classes=class_labels, candidate_frac=cutoff,
        ...
        path=f"./scripts/{scriptsbase}/in_result.{scriptformat}")
...
train_dataloader = PromptDataLoader(dataset=dataset["train"], template=mytemplate, tokenizer=tokenizer, 
    ...
    truncate_method="tail")
# print(train_dataloader.raw_dataset[0])
...
def evaluate1(prompt_model, dataloader, desc):
    ...
    # print(alllabels)
    # print(allpreds)
    news_df = pd.DataFrame(allpreds)
    # news_df.to_csv('./datasets/TextClassification/zongxiang/g10_news_label.csv')
    # news_df.to_csv('./datasets/TextClassification/zongxiang/20_news_label.csv', index=False, header=False, quoting=1)
...
if args.verbalizer == "soft":
    ...
    #     {'params': , "lr":1e-1},
    # ]
    optimizer1 = AdamW(optimizer_grouped_parameters1, lr=args.learning_rate) # 2e-5 3e-5   4e-5 5e-5
    ...
    # it's always good practice to set no decay to biase and LayerNorm z_testparameters
    optimizer_grouped_parameters1 = [
        {'params': [p for n, p in prompt_model.plm.named_parameters() if not any(nd in n for nd in no_decay)], 'weight_decay': 0.01},
        {'params': [p for n, p in prompt_model.plm.named_parameters() if any(nd in n for nd in no_decay)], 'weight_decay': 0.0}
    ]
...
for epoch in range(args.max_epochs):
    ...
    for step, inputs in enumerate(train_dataloader):
        ...
        # print(len(inputs.input_ids))  #10  ==  shot

        if use_cuda:
            inputs = inputs.cuda()
        # print(len(inputs['input_ids'][0]))  #128
